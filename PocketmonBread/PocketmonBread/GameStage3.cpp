...
void
Stage3::createStartButton(SDL_Renderer* gameRenderer)
{
	...
	this->stageButtons[STAGE_BUTTON3::RETRY_BUTTON] = new RectangleButton(texturePos, renderingPos, SDL_CreateTextureFromSurface(gameRenderer, tmpSurface));
...
Stage3::createRetryButton(SDL_Renderer* gameRenderer)
{
	...
	this->stageButtons[STAGE_BUTTON3::BACK_BUTTON] = new RectangleButton(texturePos, renderingPos, SDL_CreateTextureFromSurface(gameRenderer, tmpSurface));
	SDL_FreeSurface(tmpSurface);
}
...
{
	for (int i = 0; i < STAGE_BUTTON3::COUNT; i++)
	{
		if (this->stageButtons[i]->isClickingButtonInRange(mouseXPos, mouseYPos))
			selectButtonType(static_cast<STAGE_BUTTON3::TYPE>(i));
	}
}

void
Stage3::selectButtonType(const STAGE_BUTTON3::TYPE& buttonType)
{

	switch (buttonType)
	{
	case STAGE_BUTTON3::BACK_BUTTON:
		setNextGamePhase(GAME_PHASE::MAIN);
		break;
	case STAGE_BUTTON3::RETRY_BUTTON:
		reset();
		break;
	}
