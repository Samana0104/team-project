...
Stage3::Stage3(SDL_Window* gameWindow, SDL_Renderer* gameRenderer, Player* _gamePlayer) : gamePlayer(_gamePlayer), PhaseInterface(gameWindow, gameRenderer)
{
	...
	CH->createCharTexture(getGameRenderer(), "CharSheet.png");	
...
Stage3::~Stage3()
{
	...
	delete this->stageButtons[STAGE3_BUTTON::BACK_BUTTON];
	delete this->stageButtons[STAGE3_BUTTON::RETRY_BUTTON];
}

void Stage3::updateDatas()
{
	if (gameSituation() == 1) {
		gameStand();
	}
...
void Stage3::handleEvents(const SDL_Event& gameEvent)
{
	...
	case SDL_MOUSEBUTTONDOWN:
		...
		if (gameEvent.button.button == SDL_BUTTON_LEFT && gameSituation() == 2)
		{
			...
			this->gamePlayer->playerClearedStage3();

			if (!this->gamePlayer->isEndingViewed())
			{
				this->gamePlayer->setViewingEnding(true);
...
// init
void
Stage3::createBackGroundTexture(SDL_Renderer* gameRenderer, string BG) {
...
void
Stage3::createPlatformTexture(SDL_Renderer* gameRenderer,string platformSheet) {
...
void
Stage3::createClearMusic(string ClearBGM) {
...
void
Stage3::createCounterSound(string counter,string start) {
...
void
Stage3::createFont(SDL_Renderer* gameRenderer,string font) {
...
void
Stage3::createRetryButton(SDL_Renderer* gameRenderer)
{
	...
	this->stageButtons[STAGE3_BUTTON::RETRY_BUTTON] = new RectangleButton(texturePos, renderingPos, SDL_CreateTextureFromSurface(gameRenderer, tmpSurface));
	SDL_FreeSurface(tmpSurface);
}

void
Stage3::createBackButton(SDL_Renderer* gameRenderer)
{
	SDL_Rect texturePos = { 0, 0, 489, 121 };
	SDL_Rect renderingPos = { 607, 700, 387, 84 };

	SDL_Surface* tmpSurface = IMG_Load("../../resources/images/stage_exit_button.png");
	this->stageButtons[STAGE3_BUTTON::BACK_BUTTON] = new RectangleButton(texturePos, renderingPos, SDL_CreateTextureFromSurface(gameRenderer, tmpSurface));
	SDL_FreeSurface(tmpSurface);
}

void 
Stage3::createMouseCursor()
{
...
void 
Stage3::clickButtonsInRange(const int& mouseXPos, const int& mouseYPos)
{
	for (int i = 0; i < STAGE3_BUTTON::COUNT; i++)
	{
		if (this->stageButtons[i]->isClickingButtonInRange(mouseXPos, mouseYPos))
			selectButtonType(static_cast<STAGE3_BUTTON::TYPE>(i));
	}
	Mix_PlayChannel(1, this->buttonEffectSound, 0);
}

void
Stage3::selectButtonType(const STAGE3_BUTTON::TYPE& buttonType)
{

	switch (buttonType)
	{
	case STAGE3_BUTTON::BACK_BUTTON:
		setNextGamePhase(GAME_PHASE::MAIN);
		break;
	case STAGE3_BUTTON::RETRY_BUTTON:
		reset();
		break;
	}
